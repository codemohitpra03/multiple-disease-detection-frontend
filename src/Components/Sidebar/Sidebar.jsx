    ...
    <div className='h-[100vh] bg-[#4d1473f0] p-10 flex-col flex items-center justify-between space-y-9'>
      ...
      <div className='space-y-12'>
        ...
        <div className='space-y-2'>
          ...
          <Link to='/heart' onClick={()=>setDisease(0)} className={`flex items-center space-x-2 py-2 px-3 cursor-pointer hover:bg-[#a853e0] hover:rounded-md hover:text-gray-400 ${window.location.pathname === '/heart' ? "bg-[#a853e0] text-gray-400 rounded-md" :""}`}>
            <img width={40} src={heart} alt="" />
            <p className='text-white font-semibold text-xl'>Heart</p>
          </Link>
          <Link to='/diabetes' onClick={()=>setDisease(1)} className={`flex items-center space-x-2 py-2 px-3 cursor-pointer hover:bg-[#a853e0] hover:rounded-md hover:text-gray-400 ${ window.location.pathname === '/diabetes' ? "bg-[#a853e0] text-gray-400 rounded-md" :""}`}>
            <span>
              <img width={40} src={diabetes} alt="" />
            </span>
            <p className='text-white font-semibold text-xl'>Diabetes</p>
          </Link>
          <Link to='/parkinson' onClick={()=>setDisease(2)} className={`flex items-center space-x-2 py-2 px-3 cursor-pointer hover:bg-[#a853e0] hover:rounded-md hover:text-gray-400 ${ window.location.pathname === '/parkinson' ? "bg-[#a853e0] text-gray-400 rounded-md" :""}`}>
            <span>
