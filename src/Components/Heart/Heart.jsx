...
const Heart = () => {
  ...
  return (
    <div className='h-[100vh] bg-gray-50 p-16 overflow-auto'>
        <div id="myNav" className="overlay h-full w-0 fixed z-10 left-0 top-0 overflow-x-hidden duration-500 bg-[rgb(0,0,0,0.8)]">




            <a href="javascript:void(0)" className="closebtn absolute top-5 right-11 text-7xl text-white" onClick={closeNav} >&times;</a>
        ...
        <form onSubmit={formik.handleSubmit} autoComplete='off' >
            ...
            <button disabled={isFormValid === false} style={{color:`${isFormValid ? "white" :"gray"}`,background:`linear-gradient(90deg, rgba(207,5,189,${isFormValid ? "1" :"0.23"}) 0%, rgba(142,25,214,${isFormValid ? "1" :"0.23"}) 51%, rgba(216,0,219,${isFormValid ? "1" :"0.23"}) 100%)`}} className={`bg-[rgb(149,24,24,${isFormValid?"1":"0.23"})] md:w-[200px] flex justify-center items-center rounded-lg p-3 text-${isFormValid?"white":"[#c1c1c1]"}` }>
                {!isLoading ? "Predict Disease" : "Loading..." }
            </button>
        </form>
    </div>
  )
